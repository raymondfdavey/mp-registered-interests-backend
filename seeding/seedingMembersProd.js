...
const pathToDatesAndNamesTally = "../mp_registered_interests_data_preprocessing/datesAndNamesTally.JSON";
...
for (member in membersProductionData) {
  ...
  arrayOfMembers.push({
    ...
    categoriesOfInterestsRegistered: membersProductionData[member].categorySummary,
...
async function createMultipleListing(client, newListings) {
  const result = await client.db("registered_interests").collection("members_production").insertMany(newListings, { ordered: "false" });
...
//   const result = await client.db("registered_interests").collection("members_production").insertOne(newListing);
//   console.log(`new listing created with the following id: ${result.insertedId}`);
// }

// async function findOneListingByName(client, nameOfListing) {
//   const result = await client.db("registered_interests").collection("members_production").findOne({ name: nameOfListing });
...
//   //passing the filter first and then the set operator updates anything that is new (does not replace) but anything the same will staty the same
//   result = await client.db("registered_interests").collection("members_production").updateOne({ name: nameOfListing }, { $set: updatedListing }, { upsert: true });
...
//   result = await client.db("registered_interests").collection("members_production").deleteOne({ name: nameOfListing });
