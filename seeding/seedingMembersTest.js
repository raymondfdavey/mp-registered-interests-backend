// THIS IS NOW WORKING TO PUSH DATA TO THE DB ONLINE WITH MEMEBRS DATA

const { MongoClient, ServerApiVersion } = require("mongodb");

const fs = require("fs");

const rawMembersTestData = fs.readFileSync("./data/test_data/datesAndNamesTally.JSON");
...
async function createMultipleListing(client, newListings) {
  const result = await client.db("registered_interests").collection("members_test").insertMany(newListings);
...
async function listDatabases(client) {
  databasesList = await client.db("registered_interests").admin().listDatabases();
  console.log("Databases:");
  databasesList.databases.forEach((db) => console.log(` - ${db.name}`));
}

async function listCollections(client) {
  const collectionList = await client.db("registered_interests").listCollections().toArray();
...
//   const result = await client.db("registered_interests").collection("members_test").insertOne(newListing);
//   console.log(`new listing created with the following id: ${result.insertedId}`);
// }

// async function findOneListingByName(client, nameOfListing) {
//   const result = await client.db("registered_interests").collection("members_test").findOne({ name: nameOfListing });
...
//   //passing the filter first and then the set operator updates anything that is new (does not replace) but anything the same will staty the same
//   result = await client.db("registered_interests").collection("members_test").updateOne({ name: nameOfListing }, { $set: updatedListing }, { upsert: true });
...
//   result = await client.db("registered_interests").collection("members_test").deleteOne({ name: nameOfListing });
