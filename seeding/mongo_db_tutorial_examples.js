...
async function createListing(client, newListing) {
  const result = await client.db("mp_registered_interests").collection("members_test").insertOne(newListing);
  console.log(`new listing created with the following id: ${result.insertedId}`);
}

async function createMultipleListing(client, newListings) {
  const result = await client.db("mp_registered_interests").collection("members_test").insertMany(newListings);
...
async function findOneListingByName(client, nameOfListing) {
  const result = await client.db("mp_registered_interests").collection("members_test").findOne({ name: nameOfListing });
...
async function upsertListingByName(client, nameOfListing, updatedListing) {
  //passing the filter first and then the set operator updates anything that is new (does not replace) but anything the same will staty the same
  result = await client.db("mp_registered_interests").collection("members_test").updateOne({ name: nameOfListing }, { $set: updatedListing }, { upsert: true });
...
async function deleteListingByName(client, nameOfListing) {
  result = await client.db("mp_registered_interests").collection("members_test").deleteOne({ name: nameOfListing });
